...
router.get("/v1/dashboard", async (_req, res): Promise<void> => {
  ...
  const [todayBookingsResult] = await db
    .select({ total: count() })
    .from(bookingsTable)
    .where(and(eq(bookingsTable.isDeleted, false), gte(bookingsTable.createdAt, today)));

  const [availableDrivers] = await db
    .select({ total: count() })
    .from(driversTable)
    .where(and(eq(driversTable.isDeleted, false), eq(driversTable.status, "available")));

  const [totalVehicles] = await db
    .select({ total: count() })
    .from(vehiclesTable)
    .where(eq(vehiclesTable.isDeleted, false));

  const [activeTrips] = await db
    .select({ total: count() })
    .from(bookingsTable)
    .where(and(eq(bookingsTable.isDeleted, false), eq(bookingsTable.status, "in_progress")));

  const [pendingLeads] = await db
    .select({ total: count() })
    .from(leadsTable)
    .where(and(eq(leadsTable.isDeleted, false), eq(leadsTable.status, "new")));

  const [pendingQuotations] = await db
    .select({ total: count() })
    .from(quotationsTable)
    .where(and(eq(quotationsTable.isDeleted, false), eq(quotationsTable.status, "draft")));

  const [overdueInvoices] = await db
    .select({ total: count() })
    .from(invoicesTable)
    .where(and(eq(invoicesTable.isDeleted, false), eq(invoicesTable.status, "overdue")));

  const paidInvoices = await db
    .select()
    .from(invoicesTable)
    .where(and(eq(invoicesTable.isDeleted, false), eq(invoicesTable.status, "paid"), gte(invoicesTable.createdAt, monthStart)));
  ...
  const todayPaid = await db
    .select()
    .from(invoicesTable)
    .where(and(eq(invoicesTable.isDeleted, false), eq(invoicesTable.status, "paid"), gte(invoicesTable.createdAt, today)));
